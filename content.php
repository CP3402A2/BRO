...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	if( $wp_query->current_post == 0 && !is_paged() && is_front_page() ) { // Custom template for the first post on the front page
		if (has_post_thumbnail()) {
			...
			echo '<a href="' . get_permalink() . '" title="' . __('Read ', 'bro') . get_the_title() . '" rel="bookmark">';
	...
	?>
		<header class="entry-header">
			...
			if ( bro_categorized_blog() && is_front_page() ) {
		...
		if( $wp_query->current_post == 0 && !is_paged() && is_front_page() ) {
			...
			echo '<a href="' . get_permalink() . '" title="' . __('Read ', 'bro') . get_the_title() . '" rel="bookmark">Read the article<i class="fa fa-arrow-circle-o-right"></i></a>';
			...
			<footer class="entry-footer continue-reading">
				<?php echo '<a href="' . get_permalink() . '" title="' . __('Continue Reading ', 'bro') . get_the_title() . '" rel="bookmark">Continue Reading<i class="fa fa-arrow-circle-o-right"></i></a>'; ?>
			</footer><!-- .entry-footer -->
		<?php } ?>
	</div><!-- .index-box -->
</article><!-- #post-## -->
